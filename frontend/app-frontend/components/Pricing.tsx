          ...
          {plans.map(plan => (
            <div key={plan.nameKey} className={`${
              ...
              <div className="mx-auto max-w-xs lg:mx-0 lg:flex-auto">
                <h3 className={`text-2xl font-bold tracking-tight ${plan.popular ? 'text-white' : 'text-navy-800'}`}>
                  {t(plan.nameKey as any)}
                </h3>
                <p className={`mt-6 text-base leading-7 ${plan.popular ? 'text-blue-100' : 'text-slate-600'}`}>
                  {t(plan.descKey as any)}
                </p>
                <p className="mt-6 flex items-baseline gap-x-1">
                  <span className={`text-5xl font-bold tracking-tight ${plan.popular ? 'text-white' : 'text-navy-800'}`}>{plan.price}</span>
                  <span className={`text-sm font-semibold leading-6 ${plan.popular ? 'text-blue-100' : 'text-slate-600'}`}>{t(plan.periodKey as any)}</span>
                </p>
                <p className={`mt-3 text-sm leading-6 font-medium ${plan.popular ? 'text-cyan-200' : 'text-primary-600'}`}>
                  {t(plan.highlightKey as any)}
                </p>
                <ul role="list" className={`mt-8 space-y-3 text-sm leading-6 ${plan.popular ? 'text-blue-100' : 'text-slate-600'}`}>
                  {plan.featureKeys.map(fk => (
                    <li key={fk} className="flex gap-x-3">
                      <svg className={`h-6 w-5 flex-none ${plan.popular ? 'text-cyan-200' : 'text-primary-500'}`} viewBox="0 0 20 20" fill="currentColor">
