...
export default function AnnualPerformance({ startDate, endDate }: AnnualPerformanceProps) {
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 p-6 mb-8">
      ...
      {/* Chart — always scrollable with min width per month for readability */}
      <div className="mb-4">
        {loading ? (
          ...
          <div className="overflow-x-auto -mx-2 px-2 pb-1">
            {/* 60px per month group ensures bars are always readable */}
            <div style={{ height: 280, width: Math.max(annualData.length * 60, 320), minWidth: '100%' }}>
              <ResponsiveContainer width="100%" height={280}>
                <BarChart data={annualData} margin={{ top: 10, right: 8, left: 0, bottom: 0 }} barCategoryGap="20%" barGap={2}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" vertical={false} />
                  <XAxis dataKey="monthShort" axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: '#6b7280' }} interval={0} />
