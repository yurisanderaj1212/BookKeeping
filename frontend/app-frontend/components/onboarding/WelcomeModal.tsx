...
export default function WelcomeModal({ isOpen, onClose, onStartTour, userName }: WelcomeModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      <div className="relative bg-white dark:bg-gray-900 w-full sm:max-w-md sm:rounded-2xl rounded-t-2xl shadow-2xl border border-gray-100 dark:border-gray-800 overflow-hidden">

        {/* Header */}
        <div className="bg-linear-to-r from-primary-500 to-primary-600 px-5 py-4 text-white">
          <button onClick={onClose}
            className="absolute top-3 right-3 text-white/70 hover:text-white p-1 rounded-lg hover:bg-white dark:bg-gray-900/10 transition-colors">
            <X className="w-4 h-4" />
          </button>
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 bg-white dark:bg-gray-900/20 rounded-xl flex items-center justify-center shrink-0">
        ...
        <div className="px-5 py-4">
          <div className="text-center mb-4">
            ...
            <p className="text-gray-500 dark:text-gray-400 text-xs leading-relaxed">{t('description')}</p>
          </div>
          ...
          <div className="grid grid-cols-4 gap-2 mb-4">
            {features.map(f => (
              <div key={f.label} className="text-center p-2 bg-gray-50 dark:bg-gray-800 rounded-lg">
                ...
                <p className="text-xs font-medium text-gray-600 dark:text-gray-400 leading-tight">{f.label}</p>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-2">
            ...
            <button onClick={onClose}
              className="w-full text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:text-gray-300 transition-colors text-xs font-medium py-1.5">
              {t('btnSkip')}
            </button>
          </div>
        </div>
