      ...
      <div className="p-4 sm:p-6 border-b border-gray-200 dark:border-gray-700">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <div className="bg-green-50 dark:bg-green-900/20 p-3 sm:p-4 rounded-lg">
            ...
            <p className="text-xs text-green-600 dark:text-green-400 mt-0.5">{incomeCount} {t('transactions')}</p>
          </div>
          ...
          <div className="bg-red-50 dark:bg-red-900/20 p-3 sm:p-4 rounded-lg">
            ...
            <p className="text-xs text-red-600 dark:text-red-400 mt-0.5">{expenseCount} {t('transactions')}</p>
          </div>
          ...
          <div className={`${netProfit >= 0 ? 'bg-blue-50 dark:bg-blue-900/20' : 'bg-orange-50 dark:bg-orange-900/20'} p-3 sm:p-4 rounded-lg`}>
            ...
            <p className={`text-xs mt-0.5 ${netProfit >= 0 ? 'text-blue-600 dark:text-blue-400' : 'text-orange-600 dark:text-orange-400'}`}>
              ...
              {incomeBreakdown.length > 0 ? incomeBreakdown.slice(0, 5).map((cat: any, i: number) => (
                <div key={i} className="flex items-center justify-between p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
                  ...
                  <div className="text-right">
                    <p className="font-bold text-green-700 dark:text-green-300">{formatCurrency(cat.amount)}</p>
                    <p className="text-xs text-green-600 dark:text-green-400">{cat.percentage}%</p>
                  </div>
                </div>
              )) : (
            ...
            <div className="space-y-3">
              {expenseBreakdown.length > 0 ? expenseBreakdown.slice(0, 5).map((cat: any, i: number) => (
                <div key={i} className="flex items-center justify-between p-3 bg-red-50 dark:bg-red-900/20 rounded-lg">
                  ...
                  <div className="text-right">
                    <p className="font-bold text-red-700 dark:text-red-300">{formatCurrency(cat.amount)}</p>
                    <p className="text-xs text-red-600 dark:text-red-400">{cat.percentage}%</p>
                  </div>
                </div>
              )) : (
